...
export default function EndingWall() {
  ...
  const total = stories.reduce((n, s) => n + (s.endings?.length || 0), 0);
  ...
  const cards = useMemo(() => {
    ...
    if (sort === "story") list.sort((a, b) => a.storyTitle.localeCompare(b.storyTitle));
    return list;
  }, [owned, stories, sort]);
  ...
  const rarest = cards.length ? cards.reduce((m, c) => (c.rarity < m.rarity ? c : m), cards[0]) : null;
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <SafeAreaView edges={["top"]}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1, alignItems: "center" }}>
            ...
            <Text style={styles.subtitle}>{cards.length} of {total} · your choices, receipts</Text>
          </View>
      ...
      {cards.length === 0 ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: SPACING.xxl }}>
          {/* rarest ending banner */}
          {rarest && (
            <TouchableOpacity
              activeOpacity={0.9}
              onPress={() => share(rarest)}
              style={styles.featureBanner}
              testID="wall-rarest"
            >
              <Image source={{ uri: rarest.cover }} style={StyleSheet.absoluteFillObject} />
              <LinearGradient
              ...
              />
              <LinearGradient
                colors={[`${rarityBucket(rarest.rarity).color}55`, "transparent"]}
                start={{ x: 0, y: 0.4 }}
                end={{ x: 1, y: 1 }}
                style={StyleSheet.absoluteFill}
              />
              <View style={styles.bannerBody}>
                <View style={[styles.rarityPill, { borderColor: rarityBucket(rarest.rarity).color, backgroundColor: `${rarityBucket(rarest.rarity).color}33` }]}>
                  <Ionicons name="diamond" size={11} color={rarityBucket(rarest.rarity).color} />
                  <Text style={[styles.rarityPillText, { color: rarityBucket(rarest.rarity).color }]}>YOUR RAREST</Text>
                </View>
                <Text style={styles.bannerHeadline}>{rarest.headline}</Text>
                <Text style={styles.bannerStory}>{rarest.storyTitle} · {rarest.endingName}</Text>
                <Text style={styles.bannerRarity}>only {rarest.rarity}% of readers</Text>
            ...
            {cards.map((c, idx) => {
              ...
              return (
                <ReAnimated.View
                  ...
                  style={styles.card}
                >
                  <TouchableOpacity activeOpacity={0.88} onPress={() => share(c)} style={{ flex: 1 }} testID={`wall-card-${c.endingId}`}>
...
const styles = StyleSheet.create({
  ...
  shareRow: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 8, paddingHorizontal: 10, paddingVertical: 6, alignSelf: "flex-start", backgroundColor: "rgba(10,10,15,0.6)", borderRadius: RADIUS.pill, borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
